...
class Solver_GRASP(_Solver):
    ...
    def _selectCandidate(self, candidateList, alpha):
        ...
        minHCost = sortedCandidateList[0].cost
        maxHCost = sortedCandidateList[-1].cost
        boundaryHCost = minHCost + (maxHCost - minHCost) * alpha
        
        # find elements that fall into the RCL
        maxIndex = 0
        for candidate in sortedCandidateList:
            if candidate.cost <= boundaryHCost:
                maxIndex += 1
    ...
    def solve(self, **kwargs):
        ...
        bestHighestCost = incumbent.getCost()
        self.writeLogLine(bestHighestCost, 0)
        ...
        while not self.stopCriteria():
            ...
            if solution.isFeasible():
                solutionHighestCost = solution.getCost()
                if solutionHighestCost < bestHighestCost:
                    incumbent = solution
                    bestHighestCost = solutionHighestCost
                    self.writeLogLine(bestHighestCost, iteration)
